...
def main(args):
    ...
    for i in tqdm(range(len(testdata))):
        ...
        images = testdata[i]['image'][0].to(device)[None,...]
        with torch.no_grad():
            codedict = deca.encode(images)
            opdict, visdict = deca.decode(codedict) #tensor
            if args.render_orig:
                tform = testdata[i]['tform'][0][None, ...]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--viewer', default=True, type=lambda x: x.lower() in ['true', '1'],
                        help='OpenGL Viewer' )
